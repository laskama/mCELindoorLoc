...
class UJIdataConnector(DatasetConnector):

    def __init__(self):
        super(UJIdataConnector, self).__init__()
    ...
    def load_dataset(self):
        ...
        train = pd.read_csv(root + "/datasets/uji/trainingData.csv")

        test = pd.read_csv(root + "/datasets/uji/validationData.csv")

        data = pd.concat((train, test), axis=0)

        self.pos = data[['LONGITUDE', 'LATITUDE']].to_numpy()
        self.rss = data.iloc[:, :520].to_numpy()

        self._bld = data['BUILDINGID'].to_numpy() # np.array([building_id] * len(self.labels))
        self._floor = data['FLOOR'].to_numpy() # np.array([floor_id] * len(self.labels))
